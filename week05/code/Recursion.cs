...
public class Recursion
{
    ...
    // Problem 3: Climbing Stairs - Fixed to prevent stack overflow
    public static int CountWaysToClimb(int s, Dictionary<int, int>? remember = null)
    {
        if (remember == null)
            remember = new Dictionary<int, int>();

        // Base cases - must be comprehensive to prevent infinite recursion
        if (s < 0) return 0;  // Invalid case
        if (s == 0) return 1; // One way to stay at ground (do nothing)
        if (s == 1) return 1; // One way: take 1 step
        if (s == 2) return 2; // Two ways: 1+1 or 2
        if (s == 3) return 4; // Four ways: 1+1+1, 1+2, 2+1, 3

        // Check memoization
        if (remember.ContainsKey(s))
            return remember[s];

        // Recursive case: sum of ways to reach (s-1), (s-2), and (s-3)
